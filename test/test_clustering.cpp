...
TEST(chart_to_int_image, symbols)
{
    const land::Image<char, 1, 5, 0> chart(land::symbols, '.');
    const land::Image<int, 1, 5, 0> result = land::chart_to_int_image(chart);
...
TEST(clustering, up)
{
    ...
    EXPECT_EQ(count_clusters(clusters), 1);
...
TEST(clustering, corners)
{
    ...
    EXPECT_EQ(clusters.size(), 9);
    EXPECT_EQ(clusters.padded_size(), 25);
    EXPECT_EQ(count_clusters(clusters), 4);
